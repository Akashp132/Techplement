...
class Contact:
    def __init__(self,name,phone,email):
        ...
        self.email = email
    def __str__(self):
        return f"Name:{self.name},Phone Number:{self.phone},Email:{self.email}"
...
class contactsFunction:
    ...
    def addContacts(self,name,phone,email):
        if not self.validateName(name):
            print("Please enter alaphabets only!")
            return
    ...
    def deleteContact(self,name):
        if name in self.contacts:
            status=input("Are you sure You want to delete the contact'{name}'?(Yes/No):").strip().lower()
            if status=='yes':
